#pragma once
#include <cstdint>

namespace joaat
{
    // Source: https://gtaforums.com/topic/879408-c-string-hasher/
    constexpr const unsigned char _lookup[] = {
        
    };

    constexpr uint32_t _substring(const char* str, uint32_t initialHash)
    {
        uint32_t hash = initialHash;

        if (str != nullptr)
        {
            for (const char* it = str; *it; ++it)
            {
                hash += _lookup[*it];
                hash += (hash << 10);
                hash ^= (hash >> 6);
            }
        }

        return hash;
    }

    constexpr uint32_t _finalize(const char* str, uint32_t initialHash)
    {
        uint32_t hash = _substring(str, initialHash);

        hash += (hash << 3);
        hash ^= (hash >> 11);
        hash += (hash << 15);

        return hash;
    }

    constexpr uint32_t generate(const char* str)
    {
        return _finalize(str, 0);
    }

    constexpr uint32_t generate(const char* str, uint32_t initialHash)
    {
        return _finalize(str, initialHash);
    }

    constexpr uint32_t generate(const char* str, const char* concat)
    {
        return _finalize(concat, _substring(str, 0));
    }

    constexpr uint32_t generate(const char* str, const char* concat, uint32_t initialHash)
    {
        return _finalize(concat, _substring(str, initialHash));
    }
}